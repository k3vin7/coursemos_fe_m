...
export default function Optional_Place({
  ...
  const [center, setCenter] = useState({ lat: value?.lat ?? 37.385, lng: value?.lng ?? 127.121 });
    ...
    const getGeo = new Promise((resolve) => {
      if (secure && "geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
          (p) => { candidate = { lat: p.coords.latitude, lng: p.coords.longitude }; if (alive) setCenter(candidate); resolve(); },
          () => resolve(),
